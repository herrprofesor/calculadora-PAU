// ===============================
// script.js - versión consolidada con PAU dinámica y voluntaria corregida
// ===============================
...
const materiasModalidad = {
  ciencias: {
    ...
    segundo: [
      ...
      ["Biología", "Física"],                           // Materia de modalidad 2
...
// ----- crear selector con nota y CONV, label dinámico -----
function crearSelector(opciones, titulo, valorDefecto) {
  const div = document.createElement("div");
  div.className = "mb-2";

  const label = crearLabel(titulo + ":");
  div.appendChild(label);
  ...
  select.addEventListener("change", () => {
    label.textContent = select.value + ":"; // label dinámico
    recalcular();
  });
...
// ----- desplegables para 1º ciencias sin duplicados -----
...
function mostrarModalidad() {
  ...
  if (mod === "ciencias") {
    ...
    materiasModalidad[mod].primero.forEach(m => {
      if (Array.isArray(m)) modalidad1Div.appendChild(crearSelector(m, "Materia de modalidad"));
      else modalidad1Div.appendChild(crearCampo(m));
    });
  }
  ...
  materiasModalidad[mod].segundo.forEach((m, i) => {
    if (Array.isArray(m)) {
      ...
      if (mod === "ciencias") {
        ...
        else if (i === 1) titulo = "Materia de modalidad 2";
...
function mostrarFaseGeneral() {
  ...
  const mod = modalidadSelect.value;
  if (mod === "ciencias") faseGeneralDiv.appendChild(crearSelector(["Matemáticas II", "Matemáticas CC Sociales"], "Última materia (PAU)"));
  else if (mod === "humanidades") faseGeneralDiv.appendChild(crearCampo("Latín II"));
  else if (mod === "sociales") faseGeneralDiv.appendChild(crearCampo("Matemáticas CC Sociales"));
  else if (mod === "artes") faseGeneralDiv.appendChild(crearCampo("Dibujo Artístico II"));
  else if (mod === "musica") faseGeneralDiv.appendChild(crearSelector(["Análisis Musical II", "Artes Escénicas II"], "Última materia (PAU)"));
}
...
function recalcular() {
  ...
  volDivs.forEach((d, i) => {
    const inpNota = d.querySelector('input[type="text"]:nth-of-type(2)');
